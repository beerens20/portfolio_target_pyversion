...
def main():
    ...
    with col1:
        ...
        domestic_current = st.number_input(
            "Current Domestic Stocks/Mutual Funds ($)", min_value=0.0, value=50000.0
        )
        international_current = st.number_input(
            "Current International Stocks/Mutual Funds ($)",
            min_value=0.0,
            value=20000.0,
        )
        bonds_current = st.number_input(
            "Current Bonds ($)", min_value=0.0, value=30000.0
        )
    ...
    domestic_target, international_target = calculate_stocks(
        remaining_after_bonds, domestic_ratio=0.7
    )
    ...
    with col2:
        ...
        st.markdown(
            f"- International Stocks: ${international_target:,.2f} ({30}% of stocks)"
        )
        st.markdown(f"- Bonds: ${bond_amount:,.2f} ({age}% of total portfolio)")
